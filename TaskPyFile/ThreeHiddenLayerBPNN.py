...
#三层隐藏层BP网络初始化
def InitializeThreelayer (dimX,dimH1,dimH2,dimH3,dimY):
    learning_rate = 0.1
    w12 = 0.4*np.random.rand(dimX,dimH1)
    w23 = 0.4*np.random.rand(dimH1,dimH2)
    w34 = 0.4*np.random.rand(dimH2,dimH3)
    w45 = 0.4*np.random.rand(dimH3,dimY)
    maxEpochs = 8000
    errorThreshold = 0.1
    ActivationFunc = "relu"

    return learning_rate,maxEpochs,errorThreshold,w12,w23,w34,w45,ActivationFunc
...
#前向传播预测输出
def PredictOutput (xinput,w12,w23,w34,w45,ActivationFunc):
    if ActivationFunc == "sigmoid":
        ...
        Hidden3Out = sigmoidActivation(forwardPropagation(np.transpose(Hidden2Out),w34))
        yout = sigmoidActivation(forwardPropagation(np.transpose(Hidden3Out),w45))
        ...
        Hidden3Out = tanhActivation(forwardPropagation(np.transpose(Hidden2Out), w34))
        yout = tanhActivation(forwardPropagation(np.transpose(Hidden3Out), w45))
    elif ActivationFunc == "relu":
        Hidden1Out = reluActivation(forwardPropagation(xinput, w12))
        Hidden2Out = sigmoidActivation(forwardPropagation(np.transpose(Hidden1Out), w23))
        Hidden3Out = sigmoidActivation(forwardPropagation(np.transpose(Hidden2Out), w34))
        yout = reluActivation(forwardPropagation(np.transpose(Hidden3Out), w45))
    return yout

#反向传播计算误差并更新权值
def BackPropagation (xtrain,ytrain,w12,w23,w34,w45,learning_rate,ActivationFunc):
    ...
    if ActivationFunc == "sigmoid":
        ...
        deltaH3 = (np.dot(w45,deltaY))*Hidden3Out*(1-Hidden3Out)
        deltaH2 = (np.dot(w34,deltaH3))*Hidden2Out*(1-Hidden2Out)
        ...
        w34 = w34-learning_rate*np.dot(Hidden2Out,np.transpose(deltaH3))
        w45 = w45-learning_rate*np.dot(Hidden3Out,np.transpose(deltaY))
    elif ActivationFunc == "tanh":
        ...
        deltaH3 = (np.dot(w45, deltaY)) * (1-np.power(Hidden3Out,2))
        deltaH2 = (np.dot(w34, deltaH3)) * (1-np.power(Hidden2Out,2))
        ...
        #两个隐层和一个输出层都使用relu激活最高只能到64.10%，此函数尝试relu-sigmoid-sigmoid-relu的激活配置
        ...
        Hidden3 = forwardPropagation(np.transpose(Hidden2Out), w34)
        Hidden3Out = sigmoidActivation(Hidden3)
        y = forwardPropagation(np.transpose(Hidden3Out), w45)
        yout = reluActivation(y)

        deltaY = np.zeros((yout.shape[0],1))
        deltaH3 = np.zeros((Hidden3Out.shape[0],1))
...
if __name__ == '__main__':
    ...
    learning_rate, maxEpochs, errorThreshold, w12, w23, w34,w45, ActivationFunc = InitializeThreelayer(dimX,dimH1,dimH2,dimH3,dimY)
    ...
    for i in range (maxEpochs):
        ...
        for j in range (Xtrain.shape[0]):
            w12,w23,w34,w45,Ecost[i] = BackPropagation(np.array([Xtrain[j]]),np.transpose(np.array([Ytrain[j]])),w12,w23,w34,w45,learning_rate,ActivationFunc)
        for k in range (Xtest.shape[0]):
            Yprediction[k] = np.transpose(PredictOutput(np.array([Xtest[k]]),w12,w23,w34,w45,ActivationFunc))
